...
def load_registry() -> dict:
    ...
    if schema_version not in SUPPORTED_SCHEMA_VERSIONS:
        raise RegistryError(f"unsupported schema_version {schema_version!r}; supported: {sorted(SUPPORTED_SCHEMA_VERSIONS)}")
    ...
    try:
        ...
        raise RegistryError("plugins.json updated must be an exact YYYY-MM-DD calendar date") from error
    if parsed_updated.isoformat() != updated:
        raise RegistryError("plugins.json updated must be an exact YYYY-MM-DD calendar date")
    if document.get("registry") != CANONICAL_REGISTRY_URL:
        raise RegistryError(f"plugins.json registry URL must be {CANONICAL_REGISTRY_URL!r}")
    if document.get("registry_raw") != CANONICAL_REGISTRY_RAW_URL:
        raise RegistryError(f"plugins.json registry_raw URL must be {CANONICAL_REGISTRY_RAW_URL!r}")
    search_paths = document.get("plugin_search_paths")
    if (
        not isinstance(search_paths, list)
        or not search_paths
        or any(not isinstance(path, str) or not path.strip() or path != path.strip() for path in search_paths)
        or len(search_paths) != len(set(search_paths))
    ):
        raise RegistryError("plugins.json plugin_search_paths must be a non-empty list of unique canonical strings")
...
def validate_entry(entry: dict) -> None:
    ...
    for field in REQUIRED_FIELDS:
        ...
        if value != value.strip():
            raise RegistryError(f"entry {entry.get('name', '?')!r} field {field!r} has surrounding whitespace")

    name = entry["name"]
    if not NAME_PATTERN.match(name):
        raise RegistryError(f"plugin name {name!r} must match {NAME_PATTERN.pattern} (Zara PluginMetadata naming rules)")
    ...
    if entry["plugin_type"] not in ALLOWED_PLUGIN_TYPES:
        raise RegistryError(f"plugin {name!r} has plugin_type {entry['plugin_type']!r}; allowed: {sorted(ALLOWED_PLUGIN_TYPES)}")
    ...
    if entry["license"] != GENERATED_FLAKE_LICENSE:
        raise RegistryError(f"plugin {name!r} license must match generated flake metadata {GENERATED_FLAKE_LICENSE!r}")

    dependencies = entry.get("python_dependencies", [])
    if (
        not isinstance(dependencies, list)
        or any(not isinstance(dependency, str) or not dependency.strip() or dependency != dependency.strip() for dependency in dependencies)
        or len(dependencies) != len(set(dependencies))
    ):
        raise RegistryError(f"plugin {name!r} python_dependencies must be a list of unique canonical non-empty strings")
    ...
    if nix.get("flake") != GENERATED_FLAKE_SOURCE:
        raise RegistryError(f"plugin {name!r} nix flake must match generated registry source {GENERATED_FLAKE_SOURCE!r}")
    ...
    supported_install_commands = (
        ...
        (f"nix run {expected_target} -- install", ["nix", "run", expected_target, "--", "install"]),
    )
    if not any(install_nix == command_text and install_argv == command_argv for command_text, command_argv in supported_install_commands):
        raise RegistryError(f"plugin {name!r} install.nix must be a supported canonical generated install command for {expected_target!r}")

    plugin_dir = ROOT / entry["path"]
    if plugin_dir.parent != PLUGINS_DIR:
        raise RegistryError(f"plugin {name!r} path must live directly under plugins/ (got {entry['path']!r})")
    if plugin_dir.name != name:
        raise RegistryError(f"plugin {name!r} directory name must match the plugin name (got {plugin_dir.name!r})")
    ...
    if plugin_root.parent != PLUGINS_DIR.resolve():
        raise RegistryError(f"plugin {name!r} plugin root must resolve directly under plugins/")
    ...
    if has_installer and not installer.resolve().is_relative_to(plugin_root):
        raise RegistryError(f"plugin {name!r} installer must stay inside its plugin directory")
    ...
    if has_installer:
        ...
        if install.get("tool") != expected_tool:
            raise RegistryError(f"plugin {name!r} install.tool does not match packaged installer layout")
    ...
    if not entrypoint.is_file():
        raise RegistryError(f"plugin {name!r} entrypoint {entry['entrypoint']!r} does not exist")
    docs = entry.get("docs")
    if docs:
    ...
    if any(not tag.strip() or tag != tag.strip() for tag in tags):
        raise RegistryError(f"plugin {name!r} tags must be non-empty strings without surrounding whitespace")
...
def validate_service_entrypoint(entry: dict, entrypoint: Path) -> None:
    ...
    factories = [node for node in tree.body if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)) and node.name == "create_plugin"]
    if not factories:
    ...
    if not versions:
        versions = {value for constant_name, value in constants.items() if constant_name == "VERSION" or constant_name.endswith("PLUGIN_VERSION")}
    if versions and versions != {entry["version"]}:
        raise RegistryError(f"plugin {entry['name']!r} declares version(s) {sorted(map(str, versions))!r} but the registry publishes {entry['version']!r}")
    if not versions:
        raise RegistryError(f"plugin {entry['name']!r} version {entry['version']!r} could not be verified against the entrypoint source")

    if not api_versions:
        api_versions = {value for constant_name, value in constants.items() if constant_name == "API_VERSION" or constant_name.endswith("_API_VERSION")}
    if not api_versions:
...
def validate_catalog(document: dict) -> None:
    ...
    if unregistered:
        raise RegistryError(f"plugin directories exist without a registry entry: {sorted(unregistered)}")
