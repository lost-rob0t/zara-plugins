    ...
    let
      ...
      mkSystem = system:
        let
          ...
          pythonFor = entry:
            let dependencies = entry.python_dependencies or [ ]; in
            if dependencies == [ ] then python else
              python.withPackages (packages: map (dependency: packages.${dependency}) dependencies);

          allPluginDependencies = pkgs.lib.unique (
            pkgs.lib.concatMap (entry: entry.python_dependencies or [ ]) plugins
          );
          ...
          compatibilityPython = python.withPackages (packages:
            [ packages.langchain-core ] ++ map (dependency: packages.${dependency}) allPluginDependencies
          );

          installedCompatibilityPython = python.withPackages (packages: [ packages.langchain-core ]);
          ...
          runtimeLibraryFor = entry:
            ...
            in
            pkgs.runCommand "${entry.name}-runtime-lib" { nativeBuildInputs = [ pkgs.coreutils ]; } ''
              mkdir -p "$out"
              if [ -d ${pluginSource}/lib ]; then cp -r ${pluginSource}/lib/. "$out/"; fi
              if [ -d ${pluginPython}/${python.sitePackages} ]; then cp -rs ${pluginPython}/${python.sitePackages}/. "$out/"; fi
            '';

          mkPluginPackage = entry:
            let runtimeLibrary = runtimeLibraryFor entry; in
            pkgs.stdenv.mkDerivation {
              ...
              installPhase = ''
                ...
                ln -s $out/share/zara/plugins/${entry.name}/${entry.entrypoint} $out/share/zara/runtime/${entry.name}/entrypoint.py
          ...
          checks = {
            registry = pkgs.runCommand "zara-check-registry" { nativeBuildInputs = [ python ]; src = self; } ''
              ${python}/bin/python3 $src/scripts/validate-registry.py
              touch $out
            '';

            compatibility = pkgs.runCommand "zara-check-plugin-compatibility" { nativeBuildInputs = [ compatibilityPython ]; src = self; } ''
              export HOME=$(mktemp -d)
              export XDG_CONFIG_HOME="$HOME/.config"
              ${compatibilityPython}/bin/python3 $src/scripts/zara_compat.py --root $src --zara-source ${zaraSource}
              touch $out
            '';

            installed-compatibility = pkgs.runCommand "zara-check-installed-plugin-compatibility" { nativeBuildInputs = [ installedCompatibilityPython ]; src = self; } ''
              export HOME=$(mktemp -d)
              ${installedCompatibilityPython}/bin/python3 $src/scripts/zara_compat.py --root $src --runtime-root ${pluginEnv}/share/zara/runtime --zara-source ${zaraSource}
              touch $out
            '';

            runtime-layout = pkgs.runCommand "zara-check-runtime-layout" { nativeBuildInputs = [ python ]; }
            ...
            map
              ...
              (builtins.filter (entry: builtins.pathExists (./. + "/plugins/${entry.name}/test")) plugins)
          );
        in
        {
          packages = pluginPackages // { zara-plugins = pluginEnv; default = pluginEnv; };
          apps = { } // pkgs.lib.listToAttrs (
            map
              ...
              (builtins.filter (entry: builtins.pathExists (./. + "/plugins/${entry.name}/tools/${entry.name}")) plugins)
          ) // {
            list = { type = "app"; program = "${listApp}/bin/zara-plugins-list"; };
            default = { type = "app"; program = "${listApp}/bin/zara-plugins-list"; };
          };
          checks = checks;
          devShells.default = pkgs.mkShell {
            name = "zara-plugins-dev-shell";
            packages = [ developmentPython developmentPython.pkgs.pytest pkgs.nodejs ];
            shellHook = ''
