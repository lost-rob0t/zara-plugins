...
def run(args):
    ...
    try:
        ...
        check('edit mode removes non-touchable flag', bool(device.windows()) and all('NOT_TOUCHABLE' not in w for w in device.windows()))
        device.tap('Move avatar: toggle drag / click-through')
        check('click-through mode restored', any('NOT_TOUCHABLE' in w for w in device.windows()))
