...
PR #809 adds the default fixture and E2E gates to Companion `0.1.0-alpha.3`.
...
1. Install the exact alpha.3 debug APK through Android's installer.
...
4. Try the emotions, gestures and three dances. Set **Music genre** and **Tempo**,
   tap **Dance to genre + tempo**, then teach the local preference policy with
   **Like this move** or **Skip / teach another move**. BPM is applied to the
   procedural beat; the app does not capture ambient/playback audio.
5. Use **Idle / stop dancing and talking** or **Hide companion** to stop.
6. Use **Import VRM (maximum 32 MiB)** to test another model.
...
emotion/dance/gesture buttons, proves genre/tempo dance feedback persists in the
app-private preferences, waits for asynchronous click-through flag changes, opens
Settings underneath, hides/recreates the overlay, and revokes overlay access on
the emulator. It records screenshots.
...
`.github/workflows/companion-android.yml` runs JVM unit tests and builds/lints
the APK from the exact PR head first, then runs
...
The ordinary repository/Nix/plugin checks remain required independently. A green
main build additionally requires the update-compatible signing secret before the
rolling `companion-android-latest` GitHub prerelease can be updated.
