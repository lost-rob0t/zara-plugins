...
from zara_java_expert.plugin import MANIFEST_DIGEST, JavaExpertAdapterError, ZaraJavaExpertPlugin
...
class ContractTests(unittest.TestCase):
    ...
    def test_descriptor_matches_current_closed_contract_and_zero_model(self):
        ...
        self.assertEqual(d["expert_id"], 'zara:expert/java')
    ...
    def test_language_boundary_is_explicit(self):
        ...
        self.assertIn("inspect_jvm_project", ops); self.assertIn("compile_check", ops); self.assertNotIn("inspect_coroutines", ops)
    ...
    def test_activation_and_generation_fences(self):
        p=self.plugin()
        with self.assertRaisesRegex(JavaExpertAdapterError, "invalid-activation-id"):
            p.invoke("req:1", "activation-1", 'compile_check', 0, 0, "{\"source\": \"class X {}\"}")
        with self.assertRaisesRegex(JavaExpertAdapterError, "invalid-registry-generation"):
            p.invoke("req:1", ACTIVATION, 'compile_check', False, 0, "{\"source\": \"class X {}\"}")
    def test_stale_result_rejected(self):
        with self.assertRaisesRegex(JavaExpertAdapterError, "stale-expert-result"):
    ...
    def test_nonzero_or_boolean_model_usage_rejected(self):
        for value in (1, False):
            with self.subTest(value=value), self.assertRaisesRegex(JavaExpertAdapterError, "zero-model-proof-missing"):
    ...
    def test_read_only_effect_proof_required_and_empty(self):
        with self.assertRaisesRegex(JavaExpertAdapterError, "read-only-effect-leak"):
        ...
        with self.assertRaisesRegex(JavaExpertAdapterError, "read-only-effect-proof-missing"):
    ...
    def test_unknown_or_malformed_input_fails_before_host(self):
        runtime=FakeRuntime(); p=self.plugin(runtime)
        with self.assertRaises(JavaExpertAdapterError):
