...
class ZaraJavaExpertPlugin(ServicePlugin):
    metadata = PluginMetadata(name=PACKAGE_NAMESPACE, version=PLUGIN_VERSION, api_version="1", description='Deterministic Java/JVM syntax, package, compiler, diagnostic, style, and repair-verification adapter; Gradle/Android metadata is observation-only.')
    ...
    def descriptor(self) -> str:
        return self._json({
            ...
            "manifest_digest": MANIFEST_DIGEST,
            "name": EXPERT_NAME,
            "description": 'Deterministic Java/JVM syntax, package, compiler, diagnostic, style, and repair-verification adapter; Gradle/Android metadata is observation-only.',
            "source_reference": SOURCE_REFERENCE,
            "reasoning_kind": "symbolic",
            "operations": [_operation_descriptor(operation) for operation in sorted(ALLOWED_OPERATIONS)],
            "applicability": {"keywords": ['java', 'jvm', 'gradle', 'android']},
