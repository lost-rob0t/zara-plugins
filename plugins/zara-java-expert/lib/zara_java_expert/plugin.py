...
ALLOWED_OPERATIONS = ('applicable', 'parse', 'inspect', 'inspect_jvm_project', 'compile_check', 'diagnose', 'style', 'repair_verify', 'explain')
...
def _validate_json_tree(value: object, *, depth: int = 0, budget: list[int] | None = None) -> None:
    ...
    if budget[0] < 0 or depth > 16:
        raise JavaExpertAdapterError("input-structure-too-large")
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            ...
            _validate_json_tree(item, depth=depth + 1, budget=budget)
        return
    raise JavaExpertAdapterError("input-not-json-compatible")
...
class ZaraJavaExpertPlugin(ServicePlugin):
    ...
    def descriptor(self) -> str:
        ...
        if not isinstance(runtime_id, str) or not _TOKEN_RE.fullmatch(runtime_id):
            raise JavaExpertAdapterError("invalid-runtime-id")
