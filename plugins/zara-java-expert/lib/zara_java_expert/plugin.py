...
PACKAGE_NAMESPACE = "zara-java-expert"
EXPERT_NAME = "JavaExpert"
...
CANONICAL_SOURCE_REFERENCE = "dotfiles:.zara/experts/java"
...
def _reject_duplicate_object_pairs(
    ...
    for key, child in pairs:
        if type(key) is not str or key in value:
            raise JavaExpertAdapterError("ambiguous-input-json")
        value[key] = child
    return value
...
def _validate_source_lock() -> None:
    try:
        ...
        raise JavaExpertAdapterError("source-lock-unavailable") from error
    if "sha256:" + hashlib.sha256(raw).hexdigest() != MANIFEST_DIGEST:
        raise JavaExpertAdapterError("source-lock-digest-mismatch")
    try:
        lock = json.loads(
            raw,
        ...
        raise JavaExpertAdapterError("source-lock-invalid") from error
    if type(lock) is not dict:
        raise JavaExpertAdapterError("source-lock-invalid")
    ...
    if not (
        ...
        raise JavaExpertAdapterError("source-lock-identity-mismatch")
    ...
    try:
        specs = tuple(
            ...
            if getattr(spec, "key", None) == "java"
        )
    except Exception as error:
        raise JavaExpertAdapterError("source-lock-host-unavailable") from error
    if len(specs) != 1:
        raise JavaExpertAdapterError("source-lock-host-mismatch")
    spec = specs[0]
    if not (
        ...
        and tuple(getattr(spec, "applicability_keywords", ())) == ("java", "javac")
    ):
        raise JavaExpertAdapterError("source-lock-host-mismatch")
...
def _validate_json_tree(value: object, *, depth: int = 0) -> None:
    if depth > MAX_INPUT_DEPTH:
        raise JavaExpertAdapterError("input-structure-too-complex")
    if isinstance(value, dict):
        if len(value) > MAX_INPUT_KEYS:
            raise JavaExpertAdapterError("input-object-too-large")
        for key, child in value.items():
            if not isinstance(key, str) or not key or len(key) > 64:
                raise JavaExpertAdapterError("invalid-input-key")
            _validate_json_tree(child, depth=depth + 1)
        return
    if isinstance(value, list):
        if len(value) > MAX_INPUT_LIST:
            raise JavaExpertAdapterError("input-array-too-large")
        for child in value:
            _validate_json_tree(child, depth=depth + 1)
        return
    if isinstance(value, str):
        if len(value) > MAX_STRING_LENGTH:
            raise JavaExpertAdapterError("input-string-too-large")
        return
    if isinstance(value, float) and not math.isfinite(value):
        raise JavaExpertAdapterError("invalid-input-number")
    if value is None or isinstance(value, (bool, int, float)):
        return
    raise JavaExpertAdapterError("invalid-input-value")


def _validate_generation(value: object, field: str) -> int:
    if type(value) is not int or not 0 <= value <= MAX_GENERATION:
        raise JavaExpertAdapterError(f"invalid-{field}")
    return value
...
def _validate_operation_input(operation: str, payload: Mapping[str, object]) -> None:
    ...
    if set(payload) - set(declared):
        raise JavaExpertAdapterError("unknown-operation-input")
    for name, field in declared.items():
        if field["required"] and name not in payload:
            raise JavaExpertAdapterError("missing-operation-input")
...
def _validate_result(
    ...
    *,
    ...
    if type(invocation_id) is not str or INVOCATION_ID_RE.fullmatch(invocation_id) is None:
        raise JavaExpertAdapterError("invalid-expert-invocation-id")
    expected = {
        "protocol": PROTOCOL,
    ...
    if verdict == "succeeded" and terminal_error:
        raise JavaExpertAdapterError("terminal-fence")
    if terminal_error:
...
class ZaraJavaExpertPlugin(ServicePlugin):
    ...
    def _decode_input(input_json: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise JavaExpertAdapterError("invalid-input-json") from error
        if not isinstance(value, dict):
            raise JavaExpertAdapterError("input-must-be-object")
    ...
    def invoke(
        ...
        if result["verdict"] == "succeeded":
            ...
            if any(item.startswith("evidence:expert-provenance:sha256:") for item in evidence):
                raise JavaExpertAdapterError("provenance-evidence-namespace-conflict")
            provenance_ref = _provenance_evidence_ref()
            if provenance_ref not in evidence:
                if len(evidence) >= MAX_EVIDENCE_REFS:
                    raise JavaExpertAdapterError("provenance-evidence-budget-exceeded")
                result = dict(result)
