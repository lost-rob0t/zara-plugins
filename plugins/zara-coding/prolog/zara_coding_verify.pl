...
verify_repository(ok(Frozen), Evidence, Outcome) :-
    ...
    repository_evidence(Evidence, Root, Head, Dirty),
    maplist(repository_observation(Root, Head, Dirty),
            Requirements,
            Observations),
    zara_coding_assertions:registry(Registry),
    rlm_verify:spec_verify(Frozen, Observations, Registry, Outcome).

repository_observation(Root, Head, Dirty, Requirement, Observation) :-
    Kind = Requirement.assertion.kind,
    repository_value(Kind, Requirement.assertion.args, Root, Head, Dirty, Value),
    Observation = _{
...
repository_value(repository_head, Args, Root, Head, _, Value) :-
...
repository_value(repository_clean, Args, Root, _, Dirty, Value) :-
    get_dict(root, Args, ExpectedRoot),
    observed_text(ExpectedRoot, Root, ValueRoot),
    Value = _{root:ValueRoot,dirty:Dirty}.

repository_evidence(Evidence, Root, Head, Dirty) :-
    dict_keys(Evidence, [dirty,head,root]),
