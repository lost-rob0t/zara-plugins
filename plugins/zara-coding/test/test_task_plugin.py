...
class TaskPluginTest(unittest.TestCase):
    def test_task_tools_are_part_of_service_surface(self) -> None:
        ...
        self.assertTrue({"coding.task.create", "coding.task.get", "coding.task.record-evidence", "coding.task.complete"}.issubset(tools))
    ...
    def test_public_tool_can_record_failed_observation(self) -> None:
        ...
        class Session:
            def record_evidence(self, task_id, **kwargs):
                calls.append((task_id, kwargs)); return {"status": "ok"}
        plugin = TaskStateCodingPlugin(); plugin.task_state = Session()
    ...
    def test_task_creation_binds_fresh_repository_identity(self) -> None:
        ...
        class Inspector:
            def inspect(self, path: Path):
                calls.append(path)
                return {"root": "/projects/demo", "head": "a" * 40, "branch": "main", "dirty": False, "changed_paths": []}
        class Session:
        ...
        result = json.loads(plugin.task_create("task-1", "fix regression", repository_path="/projects/demo/src/module.py", completion_criteria=["tests"]))
    ...
    def test_missing_prolog_configuration_degrades_without_breaking_startup(self) -> None:
        ...
        self.assertEqual(status["task_state"], {"status": "unavailable", "reason": "prolog-rlm-checkout-not-configured"})
    ...
    def test_stop_clears_task_state_before_base_service_shutdown(self) -> None:
        ...
        plugin = TaskStateCodingPlugin(); session = Session(); plugin.task_state = session
        plugin.stop()
        self.assertTrue(session.stopped); self.assertIsNone(plugin.task_state); self.assertEqual(plugin.task_state_reason, "stopped")
