...
class PrologTaskStateIntegrationTest(unittest.TestCase):
    ...
    def test_task_survives_multiple_protocol_operations_and_requires_evidence(self) -> None:
        ...
        self.session.record_evidence("task-integration", kind="test", status="passed", detail="deterministic integration proof")
    ...
    def test_task_completion_requires_all_dependencies_completed(self) -> None:
        ...
        self.session.record_evidence("dependent", kind="test", status="passed", detail="dependent verifier passed")
        blocked = self.session.complete_task("dependent")
        self.session.record_evidence("dependency", kind="test", status="passed", detail="dependency verifier passed")
    ...
    def test_missing_dependency_fails_closed_at_completion(self) -> None:
        ...
        self.session.record_evidence("dependent-missing", kind="test", status="passed", detail="dependent verifier passed")
    ...
    def test_completed_task_rejects_further_mutation(self) -> None:
        ...
        self.session.record_evidence("task-terminal", kind="test", status="passed", detail="terminal proof")
    ...
    def test_task_rejects_more_than_64_evidence_records(self) -> None:
        ...
        for index in range(64):
            self.assertEqual(self.session.record_evidence("task-evidence", kind="test", status="passed", detail=f"proof-{index}")["status"], "ok")
        self.assertEqual(self.session.record_evidence("task-evidence", kind="test", status="passed", detail="overflow"), {"status": "rejected", "reason": "evidence-limit-reached"})
