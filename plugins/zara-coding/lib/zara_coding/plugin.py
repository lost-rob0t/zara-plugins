...
class ZaraCodingPlugin(ServicePlugin):
    ...
    def tools(self) -> Sequence[StructuredTool]:
        return (
            ...
            StructuredTool.from_function(func=self.git_worktree_lock, name="coding.git.worktree.lock", description="Coordination-lock one detached linked worktree at an exact observed commit with a bounded reason.", metadata=APPROVAL_METADATA),
    ...
    def git_worktree_add_detached(self, path: str, target: str, expected_head: str) -> str:
        inspector = self._require_inspector()
        self._validate_worktree_inputs(path, target, expected_head)
    ...
    @staticmethod
    def _validate_worktree_inputs(path: str, target: str, expected_head: str) -> None:
        if not isinstance(path, str) or not path:
            raise ValueError("path must be a non-empty string")
