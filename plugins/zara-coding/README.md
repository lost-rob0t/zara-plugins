...
No arbitrary shell/eval, branch overwrite/move, auto-staging, build execution, worker spawning, model-installed semantic provider, or model-driven success claim is exposed by the current surface. Worktree mutation is limited to exact detached commits inside configured roots and requires canonical approval.
