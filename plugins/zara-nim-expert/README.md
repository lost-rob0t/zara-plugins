...
`zara-nim-expert` is the Zara product adapter for **NimExpert**. Reusable semantics stay upstream in `lost-rob0t/prolog-rlm#499`; the canonical deterministic brain is pinned from `lost-rob0t/dotfiles/.zara/experts/nim` by `expert-source.lock.json`.
