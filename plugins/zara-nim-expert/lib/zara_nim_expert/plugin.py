...
PACKAGE_NAMESPACE = "zara-nim-expert"
EXPERT_NAME = "NimExpert"
...
def _decode_input(input_json: str, operation: str) -> dict[str, Any]:
    ...
    if type(payload) is not dict:
        raise NimExpertAdapterError("input-must-be-object")
    declared = {field["name"]: field for field in OPERATION_FIELDS[operation]}
    if set(payload) - set(declared):
        raise NimExpertAdapterError("unknown-operation-input")
    for name, field in declared.items():
        if field["required"] and name not in payload:
            raise NimExpertAdapterError("missing-operation-input")
        if name in payload and type(payload[name]) is not str:
            raise NimExpertAdapterError("invalid-operation-input")
    return payload
...
def _validate_result(result: Mapping[str, object], *, request_id: str, activation_id: str, operation: str, registry_generation: int, runtime_generation: int) -> None:
    ...
    if type(verdict) is not str or verdict not in RESULT_VERDICTS:
        raise NimExpertAdapterError("invalid-expert-verdict")
...
class ZaraNimExpertPlugin(ServicePlugin):
    metadata = PluginMetadata(name=PACKAGE_NAMESPACE, version=PLUGIN_VERSION, api_version="1", description="Pure-symbolic NimExpert adapter over the canonical Zara expert host")
    ...
    def invoke(self, request_id: str, activation_id: str, expert_operation: str, expected_registry_generation: int, expected_runtime_generation: int, input_json: str = "{}") -> str:
        ...
        if type(request_id) is not str or REQUEST_ID_RE.fullmatch(request_id) is None:
            raise NimExpertAdapterError("invalid-request-id")
        if type(activation_id) is not str or ACTIVATION_ID_RE.fullmatch(activation_id) is None:
            raise NimExpertAdapterError("invalid-activation-id")
        ...
        except NimExpertAdapterError:
            raise
        except Exception as error:
            raise NimExpertAdapterError("expert-host-invocation-failed") from error
        _validate_source_lock()
        ...
        if result["verdict"] == "succeeded":
            ...
            if any(item.startswith("evidence:expert-provenance:sha256:") for item in evidence):
                raise NimExpertAdapterError("provenance-evidence-namespace-conflict")
...
def create_plugin() -> ZaraNimExpertPlugin:
    return ZaraNimExpertPlugin()