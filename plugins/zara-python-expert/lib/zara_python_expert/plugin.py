...
MANIFEST_DIGEST = "sha256:b07ff1c75166954a078f6e8caafdfef83143dbcaedd46293586d0e2c037b3bbf"
...
LANGUAGE_BOUNDARIES = {"language": "python", "extensions": (".py", ".pyi"), "applicability_keywords": ("python", "py", "pyi"), "evidence_topics": ("syntax", "typing", "imports", "diagnostics", "style", "repair-verification")}
...
def _validate_source_lock() -> None:
    try:
        ...
        raise PythonExpertAdapterError("source-lock-unavailable") from error
    if "sha256:" + hashlib.sha256(raw).hexdigest() != MANIFEST_DIGEST:
        raise PythonExpertAdapterError("source-lock-digest-mismatch")
    try:
        lock = json.loads(
            raw,
        ...
        raise PythonExpertAdapterError("source-lock-invalid") from error
    if type(lock) is not dict:
        raise PythonExpertAdapterError("source-lock-invalid")
    ...
    if not (
        ...
        raise PythonExpertAdapterError("source-lock-identity-mismatch")
    try:
    ...
    try:
        ...
        raise PythonExpertAdapterError("source-lock-host-unavailable") from error
    if len(specs) != 1:
        raise PythonExpertAdapterError("source-lock-host-mismatch")
    spec = specs[0]
    if not (
...
def _reject_input_duplicate_object_pairs(
    ...
    for key, value in pairs:
        if key in result:
            raise PythonExpertAdapterError("ambiguous-input-json")
        result[key] = value
    return result
...
def _validate_result(result: Mapping[str, object], *, request_id: str, activation_id: str, operation: str, registry_generation: int, runtime_generation: int) -> None:
    ...
    if verdict == "succeeded" and terminal_error:
        raise PythonExpertAdapterError("terminal-fence")
    ...
    def invoke(self, request_id: str, activation_id: str, expert_operation: str, expected_registry_generation: int, expected_runtime_generation: int, input_json: str = "{}") -> str:
        ...
        if result["verdict"] == "succeeded":
            ...
            if any(item.startswith("evidence:expert-provenance:sha256:") for item in evidence):
                raise PythonExpertAdapterError("provenance-evidence-namespace-conflict")
            provenance_ref = _provenance_evidence_ref()
            if provenance_ref not in evidence:
                if len(evidence) >= MAX_EVIDENCE_REFS:
                    raise PythonExpertAdapterError("provenance-evidence-budget-exceeded")
                result = dict(result)
