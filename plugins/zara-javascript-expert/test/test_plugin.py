...
from zara_javascript_expert.plugin import JavaScriptExpertAdapterError, ZaraJavaScriptExpertPlugin
...
class JavaScriptExpertPluginTests(unittest.TestCase):
    operation = "inspect_module"

    def _plugin(self, runtime=None):
        plugin = ZaraJavaScriptExpertPlugin()
    ...
    def test_descriptor_is_passive_pure_symbolic(self) -> None:
        ...
        self.assertEqual(descriptor["expert_id"], "language:javascript")
    ...
    def test_invocation_is_generation_fenced_exact_zero_model(self) -> None:
        ...
        self.assertEqual(runtime.requests[0]["expert_id"], "language:javascript")
    ...
    def test_stale_generation_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(JavaScriptExpertAdapterError, "stale-or-unbound-expert-result"):
            plugin.invoke("request-1", "activation-1", self.operation, 7, 11, "{}")

    def test_effect_receipt_is_rejected(self) -> None:
        plugin = self._plugin(FakeRuntime(receipts=[{"effect": "filesystem.write"}]))
        with self.assertRaisesRegex(JavaScriptExpertAdapterError, "unexpected-side-effect-receipt"):
            plugin.invoke("request-1", "activation-1", self.operation, 7, 11, "{}")
    ...
    def test_malformed_input_fails_before_host(self) -> None:
        runtime = FakeRuntime()
        plugin = self._plugin(runtime)
        with self.assertRaisesRegex(JavaScriptExpertAdapterError, "input-must-be-object"):
            plugin.invoke("request-1", "activation-1", self.operation, 7, 11, "[]")
        self.assertEqual(runtime.requests, [])
