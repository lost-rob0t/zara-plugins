...
from zara_javascript_expert.plugin import (
    LANGUAGE_BOUNDARIES,
    MANIFEST_DIGEST,
    JavaScriptExpertAdapterError,
    ZaraJavaScriptExpertPlugin,
)
...
class ContractTests(unittest.TestCase):
    def plugin(self, runtime=None):
        value = ZaraJavaScriptExpertPlugin()
        value.start(runtime or FakeRuntime())
        return value
    ...
    def test_descriptor_uses_canonical_zero_model_language_abi(self):
        ...
        self.assertNotIn("repair.apply", CANONICAL_OPERATIONS)

    def test_javascript_boundary_is_not_typescript(self):
        self.assertEqual(LANGUAGE_BOUNDARIES["extensions"], (".js", ".jsx", ".mjs", ".cjs"))
        self.assertIn("jsx", LANGUAGE_BOUNDARIES["evidence_topics"])
        self.assertNotIn("types", LANGUAGE_BOUNDARIES["evidence_topics"])
        self.assertFalse(LANGUAGE_BOUNDARIES["jvm_project_metadata"])

    def test_inspect_crosses_only_canonical_expert_invoke_with_zero_model_budget(self):
        runtime = FakeRuntime()
        result = json.loads(self.plugin(runtime).invoke("req:1", ACTIVATION, "inspect", 1, 2, json.dumps({"source": "export const x = 1", "source_generation": "buffer:1"})))
    ...
    def test_stale_model_effect_and_old_operation_fail_closed(self):
        payload = json.dumps({"source": "export const x = 1", "source_generation": "buffer:1"})
        with self.assertRaisesRegex(JavaScriptExpertAdapterError, "stale-expert-result"):
            self.plugin(FakeRuntime(stale=True)).invoke("req:1", ACTIVATION, "inspect", 1, 2, payload)
        with self.assertRaisesRegex(JavaScriptExpertAdapterError, "zero-model-proof-missing"):
            self.plugin(FakeRuntime(model_calls=1)).invoke("req:1", ACTIVATION, "inspect", 1, 2, payload)
        with self.assertRaisesRegex(JavaScriptExpertAdapterError, "read-only-effect-leak"):
            self.plugin(FakeRuntime(receipts=[{"effect": "filesystem.write"}])).invoke("req:1", ACTIVATION, "inspect", 1, 2, payload)
        with self.assertRaisesRegex(JavaScriptExpertAdapterError, "unsupported-expert-operation"):
            self.plugin().invoke("req:1", ACTIVATION, "inspect_jsx", 1, 2, payload)
