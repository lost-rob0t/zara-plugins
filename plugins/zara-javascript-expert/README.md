...
`zara-javascript-expert` is the Zara product adapter for **JavaScriptExpert** in the pure-symbolic expert factory.

## Ownership

Reusable language semantics are not duplicated here. `lost-rob0t/prolog-rlm#500` defines the upstream contract and now hands canonical expert source ownership to `lost-rob0t/dotfiles#292`, under `.zara/experts/javascript`. This package owns only Zara lifecycle/discovery/capability adaptation.
...
JavaScript remains distinct from TypeScript; type semantics are never inferred by this adapter.
