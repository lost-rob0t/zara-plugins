...
def register_dotfiles_style_expert(
    ...
    *,
    ...
    """Register the canonical StyleExpert behind fixed transport predicates only."""
    ...
    host.preflight_registration(
        STYLE_NAMESPACE,
        files,
        predicates=_REGISTERED_PREDICATES,
    )
    host.register(
        STYLE_NAMESPACE,
        files,
...
def _validate_rule_scope_binding(
    rule: Mapping[str, Any],
    language: str,
    *,
    fence: InvocationFence,
) -> None:
    """Mirror the upstream closed style-scope binding contract at the host boundary."""
...
def _compose_dotfiles_style(
    ...
    *,
    ...
    if language not in _SUPPORTED_LANGUAGES:
        raise CompositionError(
            f"unsupported Dotfiles StyleExpert language: {language!r}"
        )
    budget.assert_zero_model_usage()
    fence.check()
...
def compose_dotfiles_style(
    ...
    *,
    ...
    """Feed canonical Dotfiles StyleExpert data into the existing Prolog-RLM resolver.

    Direct callers own their shared budget, so this API charges returned evidence
    before success. MetaExpertComposer callers should use
    DotfilesStyleCompositionInvoker, which leaves evidence accounting to the
    canonical composer exactly once.
    """
...
class DotfilesStyleCompositionInvoker:
    """Canonical MetaExpertComposer adapter for Dotfiles-owned StyleExpert data.

    The invoker is intentionally budget-neutral: MetaExpertComposer owns
    invocation/evidence charging and rejects any invoker-side budget mutation.
    The same SharedSymbolicBudget is still propagated through the registered
    predicate reads and upstream Prolog-RLM resolver for zero-model assertions.
    """
