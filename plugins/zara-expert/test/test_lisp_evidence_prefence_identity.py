...
class HostileEvidenceRef(str):
    ...
    def __str__(self):
        # str() can preserve a str subclass when __str__ returns self.
        return self

    def __eq__(self, other):
        # Forge the canonical Core evidence identity if a later validator uses
        # Python equality instead of the pre-normalization wire-type fence.
...
class LispEvidencePreNormalizationFenceTests(unittest.TestCase):
    def test_hostile_string_subclass_is_rejected_before_projection(self):
        for expert_id in DIALECTS:
            with self.subTest(expert_id=expert_id):
                ...
                with self.assertRaisesRegex(
                    CompositionError,
                    "Core Lisp expert evidence_refs must contain strings",
                ):
                    invoker(
                        expert_id,
                        "structural.check",
