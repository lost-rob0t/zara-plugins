...
class HostileVerdict(str):
    def __new__(cls):
        return super().__new__(cls, "host-controlled-status")
...
def successful_predicate_result():
    return SimpleNamespace(
        verdict=SimpleNamespace(value=HostileVerdict()),
        data={
            "result": {
                "ok": True,
...
class LispVerdictIdentityTests(unittest.TestCase):
    def test_hostile_core_verdict_string_subclass_fails_closed(self):
        for expert_id in DIALECTS:
            with self.subTest(expert_id=expert_id):
                registry = RecordingCoreRegistry(successful_predicate_result())
                ...
                with self.assertRaisesRegex(
                    CompositionError,
                    "Core Lisp expert verdict must be a built-in string",
                ):
                    invoker(
                        expert_id,
                        "structural.check",
