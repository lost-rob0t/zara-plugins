...
from zara_kotlin_expert.plugin import KotlinExpertAdapterError, ZaraKotlinExpertPlugin
...
class KotlinExpertPluginTests(unittest.TestCase):
    operation = "compile_check"

    def _plugin(self, runtime=None):
        plugin = ZaraKotlinExpertPlugin()
    ...
    def test_descriptor_is_passive_pure_symbolic(self) -> None:
        ...
        self.assertEqual(descriptor["expert_id"], "language:kotlin")
        ...
        self.assertIn("jvm-project-metadata", descriptor["required_observations"])
        self.assertIn("android-project-metadata", descriptor["required_observations"])
    ...
    def test_invocation_is_generation_fenced_exact_zero_model(self) -> None:
        ...
        self.assertEqual(runtime.requests[0]["expert_id"], "language:kotlin")
    ...
    def test_unknown_effectful_operation_rejected_before_host(self) -> None:
        runtime = FakeRuntime()
        plugin = self._plugin(runtime)
        with self.assertRaisesRegex(KotlinExpertAdapterError, "unsupported-expert-operation"):
    ...
    def test_nonzero_or_boolean_model_usage_fails_closed(self) -> None:
        for model_calls in (1, False):
            with self.subTest(model_calls=model_calls):
                ...
                with self.assertRaisesRegex(KotlinExpertAdapterError, "zero-model-proof-missing"):
    ...
    def test_stale_generation_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(KotlinExpertAdapterError, "stale-or-unbound-expert-result"):
            plugin.invoke("request-1", "activation-1", self.operation, 7, 11, "{}")

    def test_effect_receipt_is_rejected(self) -> None:
        plugin = self._plugin(FakeRuntime(receipts=[{"effect": "filesystem.write"}]))
        with self.assertRaisesRegex(KotlinExpertAdapterError, "unexpected-side-effect-receipt"):
            plugin.invoke("request-1", "activation-1", self.operation, 7, 11, "{}")
    ...
    def test_malformed_input_fails_before_host(self) -> None:
        runtime = FakeRuntime()
        plugin = self._plugin(runtime)
        with self.assertRaisesRegex(KotlinExpertAdapterError, "input-must-be-object"):
            plugin.invoke("request-1", "activation-1", self.operation, 7, 11, "[]")
        self.assertEqual(runtime.requests, [])
