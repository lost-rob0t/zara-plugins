...
from zara_kotlin_expert.plugin import LANGUAGE_BOUNDARIES, MANIFEST_DIGEST, KotlinExpertAdapterError, ZaraKotlinExpertPlugin
...
class ContractTests(unittest.TestCase):
    ...
    def test_kotlin_jvm_android_metadata_is_observation_only(self):
        self.assertEqual(LANGUAGE_BOUNDARIES["extensions"], (".kt", ".kts"))
        ...
        self.assertIn("coroutines", LANGUAGE_BOUNDARIES["evidence_topics"])
    ...
    def test_inspect_and_fences(self):
        ...
        with self.assertRaisesRegex(KotlinExpertAdapterError,"stale-expert-result"): self.plugin(FakeRuntime(stale=True)).invoke("req:1",ACTIVATION,"inspect",1,2,payload)
        with self.assertRaisesRegex(KotlinExpertAdapterError,"zero-model-proof-missing"): self.plugin(FakeRuntime(model_calls=1)).invoke("req:1",ACTIVATION,"inspect",1,2,payload)
        with self.assertRaisesRegex(KotlinExpertAdapterError,"read-only-effect-leak"): self.plugin(FakeRuntime(receipts=[{"effect":"write"}])).invoke("req:1",ACTIVATION,"inspect",1,2,payload)
        with self.assertRaisesRegex(KotlinExpertAdapterError,"unsupported-expert-operation"): self.plugin().invoke("req:1",ACTIVATION,"inspect_coroutines",1,2,payload)
