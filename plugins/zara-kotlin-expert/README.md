...
`zara-kotlin-expert` is the Zara product adapter for **KotlinExpert** in the pure-symbolic expert factory.

## Ownership

Reusable language semantics are not duplicated here. `lost-rob0t/prolog-rlm#501` defines the upstream contract and now hands canonical expert source ownership to `lost-rob0t/dotfiles#292`, under `.zara/experts/kotlin`. This package owns only Zara lifecycle/discovery/capability adaptation.
...
Kotlin remains distinct from Java. Gradle, JVM, and Android project metadata are observations, never authority.
