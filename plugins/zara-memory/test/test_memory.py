...
class MemoryServiceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.memory.register_schema(
            MemorySchema(
                ...
                allowed_fact_predicates=frozenset({"workflow_state", "verification"}),
            )
        )

    def test_scopes_are_isolated_by_scope_and_owner(self):
        one = self.memory.remember(
            scope="project",
            owner="repo-a",
            text="Use Prolog verification",
    ...
    def test_full_text_provenance_and_stable_id_are_preserved(self):
        item = self.memory.remember(
            scope="project",
            owner="repo-a",
            text="Exact text — keep punctuation & whitespace.",
            facts=["verification(prolog)"],
            provenance={"source": "operator", "message_id": "m-1"},
            memory_type="coding.workflow",
        )
        recalled = self.memory.recall(scope="project", owner="repo-a")
    ...
    def test_all_initial_scopes_are_explicitly_supported(self):
        self.assertEqual(
            self.memory.supported_scopes,
            frozenset({"session", "user", "project", "machine", "global"}),
        )
