...
`zara-prolog-expert` is the Zara product adapter for **PrologExpert**. Reusable semantics stay upstream in `lost-rob0t/prolog-rlm#495`; the canonical deterministic brain is pinned from `lost-rob0t/dotfiles/.zara/experts/prolog` by `expert-source.lock.json`.
