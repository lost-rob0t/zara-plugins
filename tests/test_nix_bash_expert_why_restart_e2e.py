"""Gate persisted Nix/Bash expert evidence into real symbolic why follow-ups.

The reusable symbolic dialogue layer already knows how to explain a prior typed
expert answer without a model. This acceptance proves product composition can
recover both NixExpert and BashExpert answers from Zara's canonical conversation
projection after process recreation and route ``why?`` through the same symbolic
runtime. It deliberately adds no conversation store, expert registry, or
provider path.
"""
...
class NixBashExpertWhyRestartE2ETests(durable.NixBashDurablePersistenceE2ETests):
    ...
    async def _why_turn(self, store: Any, conversation_id: str, turn_id: str) -> Any:
    ...
    def _assert_expert_restart(
        ...
        *,
        ...
        selected = next(item for item in expert_evidence if item["expert_id"] == expert_id)
        ...
        self.assertEqual(selected["effect_receipts"], [])

        database_path = root / f"conversation-{language}.db"
        database = DatabaseManager(database_path)
        store = ConversationStore(database)
        conversation_id = f"conv-{language}-expert-why-restart"
        conversation = store.create_conversation(
            f"{expert_name} why restart",
            conversation_id=conversation_id,
        )
        prior_act = (
            f"answer(expert,\"{expert_name} inspected source.\",evidence('"
            + evidence_ref
            + "'))"
        )
        seeded = store.save_symbolic_projection(
            SymbolicConversationProjection(
                ...
                turn_id=f"turn-{language}-expert-answer",
                outcome="success",
        ...
        try:
            ...
            self.assertEqual(
                ...
                f"persisted {expert_name} answer was not supplied to symbolic follow-up",
            )
    ...
    def test_persisted_expert_answers_route_why_through_symbolic_followup(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for language, expert_name in (
                ("nix", "NixExpert"),
                ("bash", "BashExpert"),
            ):
                with self.subTest(language=language):
                    self._assert_expert_restart(
                        root=root,
                        expert_evidence=expert_evidence,
                        language=language,
